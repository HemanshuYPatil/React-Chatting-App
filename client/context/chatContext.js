...
export const ChatProvider = ({ children }) => {
  ...
  const handleSelectedChat = async (chat) => {
    ...
    if (!chat) {
      setActiveChatData(null); // Clear active chat data when chat is null
      return;
    }
  
    if (!chat.participants) {
      console.log("Chat participants are null or undefined.");
      return;
    }
    ...
    if (isNotLoggedInUser) {
      ...
      console.log("No other participants found in the chat.");
      setActiveChatData(null); // Clear active chat data if no other participants
    }
  };
